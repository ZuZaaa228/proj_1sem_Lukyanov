...
min_index = str(
    all_elements.split().index(str(min([int(x) for x in all_elements.split()]))))  # Индекс минимального числа файла 1
...
max_index = str(
    file2_elements.split().index(str(max([int(x) for x in file2_elements.split()]))))  # максимальный индекс файла 2
